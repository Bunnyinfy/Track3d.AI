...
def save_current_project(user_id, project_name=None):
    ...
    project_id = save_user_project(
        user_id, project_name, st.session_state.project_specs
    )
...
def update_project(project_id, user_id, project_name=None):
    ...
    success = update_user_project(
        project_id, user_id, project_name, st.session_state.project_specs
    )
...
def display_user_projects(user_id):
    ...
    with cols[0]:
        selected_project_id = st.selectbox(
            ...
            format_func=lambda x: next(
                (p["name"] for p in projects if p["id"] == x), ""
            ),
        )
    ...
    with cols[1]:
        if st.button("Load Project", key="load_project_button"):
            if selected_project_id:
                ...
                if success:
                    st.success(message)
                    # Switch to Project Specifications tab after loading
    ...
    # # Display loaded project details if available
    # if "project_specs" in st.session_state and st.session_state.project_specs:
    #     st.subheader("Loaded Project Details")
    #     specs = st.session_state.project_specs

    #     # Display main specs
    #     st.markdown(f"**Applications:** {', '.join(specs.get('applications', []))}")
    #     st.markdown(f"**Material Types:** {', '.join(specs.get('material_types', []))}")
    #     st.markdown(
    #         f"**Minimum Strength (MPa):** {specs.get('min_strength_mpa', 'N/A')}"
    #     )
    #     st.markdown(
    #         f"**Minimum Durability (years):** {specs.get('min_durability_years', 'N/A')}"
    #     )
    #     st.markdown(
    #         f"**Fire Resistance Requirement (hours):** {specs.get('fire_resistance_requirement', 'N/A')}"
    #     )
    #     st.markdown(
    #         f"**Water Resistance Requirement (1-10):** {specs.get('water_resistance_requirement', 'N/A')}"
    #     )
    #     st.markdown(
    #         f"**Thermal Requirement:** {specs.get('thermal_requirement', 'N/A')}"
    #     )
    #     st.markdown(
    #         f"**Eco-Friendly Requirement (1-10):** {specs.get('eco_friendly_requirement', 'N/A')}"
    #     )
    #     st.markdown(
    #         f"**Budget Constraint ($ per unit):** {specs.get('budget_constraint', 'N/A')}"
    #     )
    #     st.markdown(
    #         f"**Installation Time Constraint:** {specs.get('installation_time_constraint', 'N/A')}"
    #     )

    #     # Environmental conditions
    #     env = specs.get("environmental_conditions", {})
    #     st.markdown("**Environmental Conditions Importance:**")
    #     st.markdown(f"- Heat: {env.get('heat', 'N/A')}")
    #     st.markdown(f"- Cold: {env.get('cold', 'N/A')}")
    #     st.markdown(f"- Humidity: {env.get('humidity', 'N/A')}")
    #     st.markdown(f"- UV: {env.get('uv', 'N/A')}")
            ...
            if st.button("Update Project", key="update_project_button"):
                if "project_specs" in st.session_state:
                    success, message = update_project(
                        selected_project[0],
                        user_id,
                        project_name or selected_project[1],
                    )
                    if success:
                        st.success(message)
                    else:
                    ...
                    st.warning(
                        "Please configure project specifications before updating."
                    )
