...
def preprocess_material_data(materials_df):
    ...
    for prop in weather_props:
        X[f'weather_{prop}'] = materials_df['weather_resistance'].apply(lambda x: x.get(prop, 0))
    ...
    for app in set(app for apps in materials_df['applications'] for app in apps):
        app_dummies[f'app_{app}'] = materials_df['applications'].apply(lambda x: 1 if app in x else 0)
...
def extract_project_features(project_specs):
    ...
    # Process numerical requirements
    numerical_features = [
        'min_strength_mpa',
...
def calculate_material_scores(materials_df, project_specs):
    ...
    scored_materials['application_score'] = scored_materials['applications'].apply(
        lambda x: sum(1 for app in requested_applications if app in x) / max(1, len(requested_applications))
    ) * 10
    scored_materials['total_score'] += scored_materials['application_score'] * weights['application_match']
    ...
    if preferred_types:
        ...
        scored_materials['total_score'] += scored_materials['type_score'] * weights['strength']
    ...
    if min_strength > 0:
        ...
        scored_materials['total_score'] += scored_materials['strength_score'] * weights['strength']
    ...
    if min_durability > 0:
        ...
        scored_materials['total_score'] += scored_materials['durability_score'] * weights['durability']
    ...
    if fire_req > 0:
        ...
        scored_materials['total_score'] += scored_materials['fire_score'] * weights['fire_resistance']
    ...
    if water_req > 0:
        ...
        scored_materials['total_score'] += scored_materials['water_score'] * weights['water_resistance']
    ...
    if thermal_req is not None:
        if thermal_req == 'low':  # Good insulation
            scored_materials['thermal_score'] = 10 - (scored_materials['thermal_conductivity'] / 10)
        else:  # High conductivity
            scored_materials['thermal_score'] = scored_materials['thermal_conductivity'] / 10
        
        scored_materials['thermal_score'] = scored_materials['thermal_score'].clip(0, 10)
        scored_materials['total_score'] += scored_materials['thermal_score'] * weights['thermal']
    ...
    if eco_req > 0:
        ...
        scored_materials['total_score'] += scored_materials['eco_score'] * weights['eco_friendly']
    ...
    if budget is not None:
        ...
        scored_materials['total_score'] += scored_materials['cost_score'] * weights['cost']
    ...
    if env_conditions:
        ...
        for condition, importance in env_conditions.items():
            if importance > 0 and condition in ['heat', 'cold', 'humidity', 'uv']:
                weather_score += scored_materials['weather_resistance'].apply(
        ...
        if count > 0:
            ...
            scored_materials['total_score'] += scored_materials['weather_score'] * weights['weather']
    ...
    if install_constraint is not None:
        if install_constraint == 'low':
            scored_materials['install_score'] = 10 - scored_materials['installation_complexity']
        else:
            scored_materials['install_score'] = scored_materials['installation_complexity']
        
        scored_materials['total_score'] += scored_materials['install_score'] * weights['installation']
    
    # Normalize the total score
    total_weight = sum(w for k, w in weights.items())
    scored_materials['total_score'] = scored_materials['total_score'] / total_weight * 10
