...
class MaterialRecommender:
    ...
    def get_similar_materials(self, material_id, n_neighbors=3):
        ...
        material_idx = self.materials_df[self.materials_df['id'] == material_id].index[0]
        
        # Get the feature vector for the material
        material_features = self.scaled_features[material_idx].reshape(1, -1)
        
        # Find the nearest neighbors
        distances, indices = self.knn_model.kneighbors(material_features, n_neighbors=n_neighbors+1)
    ...
    def predict_material_ratings(self, project_specs):
        ...
        rated_materials = rated_materials.sort_values(by='predicted_rating', ascending=False)
